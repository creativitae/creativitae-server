...
const { Customer, Template, Admin, CustomerTemplate, CustomerDetail } = require("../../models");
...
afterAll(async () => {
  ...
  await CustomerDetail.destroy({ truncate: true, cascade: true, restartIdentity: true });
});
describe("GET /public/mydetail", () => {
  it("should return 200 status code", async () => {
    ...
    expect(response.body).toHaveProperty("workExperiences", myDetail.workExperiences)
    ...
    expect(response.body).toHaveProperty("certifications", myDetail.certifications)
    ...
    expect(response.body.Customer).toHaveProperty("username", expect.any(String))
    expect(response.body.Customer).toHaveProperty("email", expect.any(String))
    expect(response.body.Customer).toHaveProperty("password", expect.any(String))
    expect(response.body.Customer).toHaveProperty("phoneNumber", expect.any(String))
    expect(response.body.Customer).toHaveProperty("address", expect.any(String))
    expect(response.body.Customer).toHaveProperty("createdAt", expect.any(String))
    expect(response.body.Customer).toHaveProperty("updatedAt", expect.any(String))
  });
