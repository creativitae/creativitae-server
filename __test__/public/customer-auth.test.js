...
describe("POST /public/register", () => {
  it("should return 201 status code if request body is valid", async () => {
    ...
    const response = await request(app).post("/public/register").send(customer);
    expect(response.status).toBe(201);
    expect(response.body.customer).toHaveProperty("id", expect.any(Number));
    expect(response.body.customer).toHaveProperty("email", customer.email);
    expect(
      compare(customer.password, response.body.customer.password)
    ).toBeTruthy();
  });
...
describe("POST /public/login", () => {
  it("should return 200 status code if request body is valid", async () => {
    ...
    const response = await request(app).post("/public/login").send(customer);
